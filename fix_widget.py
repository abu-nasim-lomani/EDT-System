...
# Fix 1: Django tags
# First remove extra endif at end:
html = re.sub(r'{%\s*endif\s*%}\s*{%\s*endif\s*%}\s*$', '{% endif %}\n', html)
# Then fix split if tag (assume it might be split across lines)
...
html = html.replace('''<input type=\"text\" x-model=\"inputMessage\" placeholder=\"Type a message…\"
                                    :disabled=\"isLoading\"
...
'''<textarea x-model=\"inputMessage\" placeholder=\"Type a message…\"
                                    :disabled=\"isLoading\"
                                    ...
                                    onfocus=\"this.style.borderColor='#6366f1';this.style.background='#fff';\"
                                    onblur=\"this.style.borderColor='#e2e8f0';this.style.background='#f8fafc';\"></textarea>'''.replace('`$', '$'))
...
with open('chatbot/templates/chatbot/chat_widget.html', 'w', encoding='utf-8') as f:
    f.write(html)
print('Rewrite logic completed safely using Python')
